...
// ── Notify owner of a single new eBay listing needing approval ────────────────
// Used by trigger-worker.js (the live discovery engine). Mirrors the same
// approve/reject button format as sendBatchToTelegram() above, so webhook.js
// (which already knows how to parse this exact callback_data shape) handles
// the response with zero changes needed on that side.
//
// FIX (2026-06-29): trigger-worker.js was writing status:'live' directly and
// never called any notify function — so nothing ever reached Telegram for
// approval, even after the collection-mismatch fix made discovery itself
// start working. This closes that gap: products now land as status:'pending'
// and the owner approves/rejects from Telegram, matching what the dashboard
// UI already promises ("sent to Telegram for approval").
export async function notifyProductForApproval(product, jobId) {
  if (!BOT || !CHAT) return

  const text = [
    `🆕 ${product.name}`,
    ``,
    `🏷️ Category: ${product.category}${product.isAccessory ? ' (accessory)' : ''}`,
    `💰 Sell price: ${product.sellingPriceFormatted || product.sellingPriceSAR + ' SAR'}`,
    `🏪 Source price: ${product._sourcePriceSAR ? product._sourcePriceSAR + ' SAR' : 'N/A'}`,
    `🔗 ${product._sourceLink || 'No link'}`,
  ].join('\n')

  const keyboard = {
    inline_keyboard: [[
      { text: '✅ Approve & Publish', callback_data: `approve_${product.id}_${jobId}` },
      { text: '❌ Reject',            callback_data: `reject_${product.id}_${jobId}` },
    ]],
  }

  if (product.image) {
    const photoResult = await tgPhoto(product.image, text, keyboard)
    if (!photoResult.ok) await tgKeyboard(text, keyboard)
  } else {
    await tgKeyboard(text, keyboard)
  }
}
