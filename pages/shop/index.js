 ...
 * FIXED: Token handling + year updated to 2026
 */
...
export default function ShopDashboard() {
  ...
  const [query,      setQuery]      = useState(
    'best selling trending products Saudi Arabia 2026'
  )

  useEffect(() => {
    const saved = sessionStorage.getItem('husin_shop_pwd')
  ...
  async function handleLogin(e) {
    ...
    try {
      ...
      if (data.success) {
        // Store raw password — used directly as token for simplicity
        sessionStorage.setItem('husin_shop_pwd', password)
        setAuthed(true)
      } else {
  ...
  async function loadStats() {
    try {
      const pwd = sessionStorage.getItem('husin_shop_pwd')
      const res = await fetch('/api/shop/stats', {
        headers: { 'x-shop-token': pwd }
      })
  ...
  async function triggerSearch() {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setTriggerMsg({ type: 'error', text: `❌ ${data.error || 'Unknown error'}` })
      }
    } catch (e) {
  ...
  function logout() {
    sessionStorage.removeItem('husin_shop_pwd')
    setAuthed(false)
    setPassword('')
    setStats(null)
  }
      ...
      <div style={s.content}>
          ...
          <div style={s.statsGrid}>
            {[
              ...
              <div key={i} style={s.statCard}>
                ...
                <span style={{ ...s.statNum, color: st.color }}>
                  {Number(st.value || 0).toLocaleString()}
                </span>
        ...
        <div style={s.panel}>
          ...
          <p style={s.panelDesc}>
            ...
            15 famous (Amazon.sa, Noon, Namshi, Farfetch, NET-A-PORTER…) +
            15 hidden gems (Trendyol, Banggood, YesStyle, Desertcart, Ubuy…).
        ...
        <div style={s.quickGrid}>
          {[
            { href: '/shop/inbox',  icon: '📥', title: 'Product Inbox',    desc: 'Review 150 products — approve or reject each one' },
            { href: '/marketplace', icon: '🛒', title: 'Live Marketplace', desc: 'What your customers see — approved products displayed here', blank: true },
            { href: '#', icon: '📦', title: 'Orders',   desc: 'Manage customer orders (coming in Batch 4)',   disabled: true },
            { href: '#', icon: '💳', title: 'Payments', desc: 'Stripe & PayPal setup (coming in Batch 4)', disabled: true },
          ].map((card, i) => (
            <a
              key={i}
            ...
            >
              ...
              {card.disabled && <span style={s.comingSoon}>Coming Soon</span>}
            </a>
          ))}
        </div>
      </div>
...
const s = {
  ...
  triggerBtn:   { background:'#00d9ff', color:'#000', border:'none', borderRadius:8, padding:'13px 28px', fontSize:'0.95rem', fontWeight:700, cursor:'pointer', whiteSpace:'nowrap', fontFamily:'inherit', transition:'opacity 0.2s' },
  ...
  quickCard:    { background:'#0f1117', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, padding:24, textDecoration:'none', display:'flex', flexDirection:'column', gap:8, position:'relative' },
