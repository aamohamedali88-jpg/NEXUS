/**
 * HUSIN ESHOP — /marketplace — HOTFIX
 * FIX 1: Sidebar hidden on mobile using GLOBAL style (not scoped JSX)
 *         Scoped JSX styles don't override component-level styles reliably
 * FIX 2: Grid takes full width when sidebar is hidden
 */
...
export default function Marketplace({ products, total }) {
  ...
  return (
    <>
      ...
      {/*
        FIX: All responsive CSS is in a GLOBAL style block
        Global styles are not scoped and reliably override all element styles
        This is the only reliable way to hide the sidebar on mobile in Next.js
      */}
      <style>{`
        @keyframes shimmer {
        ...
        /* Mobile pills - hidden by default on desktop */
        ...
        @media (max-width:768px) {
          ...
          /* HIDE sidebar completely - using multiple selectors for maximum specificity */
          .mp-sidebar,
          aside.mp-sidebar,
          .mp-body .mp-sidebar,
          ...
          /* Grid area takes FULL width */
          .mp-grid-area { width:100% !important; flex:1 !important; }

          /* Body has no gap since sidebar is gone */
